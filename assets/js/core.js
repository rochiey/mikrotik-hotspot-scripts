...
$(document).ready(function(){
  ...
  if(dataRateOption){
	$("#dataInfoDiv").css('display', '');
	$("#dataInfoDiv2").css('display', '');
 }
  ...
  if( qrCodeVoucherPurchase ){
	  $("#scanQrBtn").attr("style", "display: inline-block");
  }
           ...
           success: function(data, textStatus, request){
              if ( data.indexOf("IAMNOTLOGINSTRINGPLEASEDONTREMOVE") < 0 ){
				...
				if(EnableTelegram == true) {
					...
					if (isMultiVendo) {
						if (multiVendoOption == 1){
							var teleMsg ='********* <b>Insert Coin Active</b> ********%0AVendo Name: '+ VendoLoc[loc].name + '%0AVendo IP: ' + vendorIpAddress + '%0A************ <b>Client Info</b> *************' + '%0AMAC: ' + mac + '%0AIP: ' + dev_ip + '%0A*************************************';
						}else if(multiVendoOption == 2){
							var teleMsg ='********* <b>Insert Coin Active</b> ********%0AVendo Name: '+ VendoLoc[loc].name + '%0AVendo IP: ' + vendorIpAddress + '%0A' + '%0A************ <b>Client Info</b> *************' + '%0AMAC: ' + mac + '%0AIP: ' + dev_ip + '%0A*************************************';	
						}else{
							var teleMsg ='********* <b>Insert Coin Active</b> ********%0AVendo Name: '+ vendoipAddr + '%0AVendo IP: ' + vendorIpAddress + '%0A' + '%0A************ <b>Client Info</b> *************' + '%0AMAC: ' + mac + '%0AIP: ' + dev_ip + '%0A*************************************';
						} 
					}else{
	...
	$.ajax({
	  ...
	  success: function(data){
		...
		for(r in rows){
			...
			rates = rates + "</div>";
			rates = rates + "<div class='rdata'><span style='color: #c1a980'>Validity: ";
			rates = rates + secondsToDhms(parseInt(columns[3])*60);
			rates = rates + "</span></div>";
			if(dataRateOption){
				rates = rates + "<div class='rdata'><span style='color: #c1a980'>Data: ";
				if(columns[4] != ""){
	...
	$.ajax({
	  ...
	  success: function(data){
		...
		for(r in rows){
			...
			chargingStation = chargingStation + "<div class='rdata'><span style='color: #c1a980'>Status: <span name='portStatus'>-";
			chargingStation = chargingStation + "</span></span></div>";
			chargingStation = chargingStation + "<div class='rdata'><span style='color: #c1a980'>Remaining: <span name='remainTime'>-";
			chargingStation = chargingStation + "</span></span></div>";
			chargingStation = chargingStation + "<div class='rdata'><span style='color: #c1a980'>";
			chargingStation = chargingStation + "<button class='btn btn-green' style='display: none' name='useBtn' onClick=\"addChargerTime("+r+", \'"+columns[0]+"\',0)\">Avail</button>";
	  ...
	  beforeSend: function(){
		if(EnableTelegram == true && totalCoinReceived > 0) {
			...
			var loc;
					...
					if (isMultiVendo) {	
							if (multiVendoOption == 1){
											var teleMsg = '********** <b>Insert Coin Done</b> *********%0ATotal Coin: ' + '<b>'+totalCoinReceived + '.00 php'+'</b>' + '%0AVoucher: ' + '<b>'+voucher+'</b>' + '%0AVendo Name: ' + VendoLoc[loc].name + '%0AVendo IP: ' + vendorIpAddress + '%0A' + '%0A************ <b>Client Info</b> ************' + '%0AMAC: ' + mac + '%0AIP: ' + dev_ip + '%0A*************************************';
							}else if(multiVendoOption == 2){
											var teleMsg = '********** <b>Insert Coin Done</b> *********%0ATotal Coin: ' + '<b>'+totalCoinReceived + '.00 php'+'</b>' + '%0AVoucher: ' + '<b>'+voucher+'</b>' + '%0AVendo Name: ' + VendoLoc[loc].name + '%0AVendo IP: ' + vendorIpAddress + '%0A' + '%0A************ <b>Client Info</b> ************' + '%0AMAC: ' + mac + '%0AIP: ' + dev_ip + '%0A*************************************';
							}else{
											var teleMsg = '********** <b>Insert Coin Done</b> *********%0ATotal Coin: ' + '<b>'+totalCoinReceived + '.00 php'+'</b>' + '%0AVoucher: ' + '<b>'+voucher+'</b>' + '%0AVendo Name: ' + vendoipAddr + '%0AVendo IP: ' + vendorIpAddress + '%0A' + '%0A************ <b>Client Info</b> ************' + '%0AMAC: ' + mac + '%0AIP: ' + dev_ip + '%0A*************************************';
							}
					}else {
...
function notifyCoinSuccess(coin){
	...
	if(EnableTelegram == true && CoinDropNotify) {
        ...
        if (isMultiVendo) {
			if (multiVendoOption == 1){
					var teleMsg = '******** <b>Inserted Coin Count</b> *******%0AAmount: ' + "<b>"+coin + '.00 php'+ '</b>' + '%0AMAC: ' + mac + '%0AIP: ' + dev_ip + '%0AVendo Name: ' + VendoLoc[loc].name + '%0AVendo IP: ' + vendorIpAddress + '%0A*************************************';
			}else if(multiVendoOption == 2){
					var teleMsg = '******** <b>Inserted Coin Count</b> *******%0AAmount: ' + "<b>"+coin + '.00 php'+ '</b>' + '%0AMAC: ' + mac + '%0AIP: ' + dev_ip + '%0AVendo Name: ' + VendoLoc[loc].name + '%0AVendo IP: ' + vendorIpAddress + '%0A*************************************';
			}else{
					var teleMsg = '******** <b>Inserted Coin Count</b> *******%0AAmount: ' + "<b>"+coin + '.00 php'+ '</b>' + '%0AMAC: ' + mac + '%0AIP: ' + dev_ip + '%0AVendo Name: ' + vendoipAddr + '%0AVendo IP: ' + vendorIpAddress + '%0A*************************************';
			} 
		}else {
...
if (EnableAnnouncement){
	$("#announcement-holder").css('display', '');
	$("#announcement").html(annoucementText);
}else{
$("#announcement-holder").attr("style", "display: none!important");
}

// Footer Brand Name-----------------------------------------------
$('#footerName').html(FooterName);

// Trial Claim hide InsertBtn Multivendo Pause --------------------
if (window.location.href.indexOf("/status") > -1) {
	if ($('#uvc').html() != null){
		var uvc = $('#uvc').html();
		if (uvc.startsWith("T-")){
			$("#insertBtn").attr("style", "display: none!important");
			$("#pauseTimeBtn").attr("style", "display: none!important");
			$("#vendoSelectDiv").attr("style", "display: none!important");
			$("#trialText").css('display', '');
